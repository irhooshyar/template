...
def apply(folder_name, Country):
    # reading result
    file = open(r"G:\tabnak\result_tabnak_2.txt", "r", encoding="utf-8")
    delimiter = "&!&"
    text = file.read()
    items = text.split("\n")
    counter = 1
    ...
    for item in items:
        ...
        if itemArray.__len__() > 4:
            pId = itemArray[1]
            if pId:
                pTagRes = itemArray[4].replace('"',"").replace("'", '"').replace("\\", '\\\\')
                pClassification = itemArray[3]
                ...
                try:
                    ...
                    object = FullProfileAnalysis(country_id=Country.id, document_paragraph_id=int(pId),
